...
#include<stdio.h>
#include<stdlib.h>
int Touch(SOUP* pCardGroup, SOUP *TEmporary,int *intCardNum, int NNN,char* pGameSeria,SOUP Output1[13], char* pGameSeria2, SOUP Output2[13], char* pGameSeria3, SOUP Output3[13], char* pGameSeria4, char* szQI,char* szBarRecord)
{
	...
	if (intLS == 3)
	{
		...
		printf("玩家可以杠/碰牌，是否杠牌（1确认碰，0取消碰,取消杠判定后进行碰判定）\n");
		ChOutput(chSort, *intCardNum, 2, 1, intLS1 - 1, intLS1+1, pGameSeria,NULL, NULL, NULL, NULL, NULL, NULL, NULL,NULL);
		for (;;)
		{
			intKZ = getchar() - '0';
		...
		if (intLS == 2&& NNN != 1)
		{
			...
			ChOutput(chSort, *intCardNum,2,1, intLS1-1, intLS1, pGameSeria, NULL, NULL, NULL, NULL, NULL, NULL, NULL,NULL);
			for (;;)
			{
				intKZ = getchar() - '0';
				if (intKZ == 1 || intKZ == 0)break;
			...
			if (intKZ == 1)
			{
				...
				*intCardNum -= 3;
				...
				chSort[13][0] = TEmporary->inSerial + '0'; chSort[13][1] = TEmporary->chSign; chSort[13][2] = '\0';
				intLS2 = ChOutput(chSort, *intCardNum+1, 0, 0, 3, 0, pGameSeria, Output1, pGameSeria2, Output2, pGameSeria3, Output3, pGameSeria4, szQI, szBarRecord);
...
int OWTouch(SOUP* pCardGroup, SOUP TEmporary ,int *intCardNum)//加杠
{
	...
	for (int i = 0; i < *intCardNum - 3; i++)
	{
			...
			pCardGroup[i].inSerial == pCardGroup[i + 1].inSerial && pCardGroup[i].inSerial == pCardGroup[i + 3].inSerial && pCardGroup[i].inSerial == pCardGroup[i + 2].inSerial)
		{
			...
			ChOutput(chSort, 14, i, 1, i, i + 3, "", NULL, NULL, NULL, NULL, NULL, NULL, NULL,NULL);
			for (;;)
			{
				intKZ = getchar() - '0';
				if (intKZ == 1 || intKZ == 0)break;
			...
			if (intKZ == 1)
			{
				...
				pCardGroup[i].inSerial = TEmporary.inSerial; pCardGroup[i].chSign = TEmporary.chSign;
				TEmporary.inSerial = pCardGroup[*intCardNum - 1].inSerial; pCardGroup[i].chSign = pCardGroup[*intCardNum - 1].chSign;
				intCardNum -= 3; break;
			}
		}
	}
	for (int i = 0; i < 13; i++)
	{
		if (pCardGroup[i].chSign == TEmporary.chSign && pCardGroup[i].inSerial == TEmporary.inSerial&& i>= intCardNum)
			intLS++;
		if (n != 0)
		{
			if (intLS == 1)
	...
	if (intLS == 3)
	{
		...
		ChOutput(chSort, 14, 2, 1, intLS1 - 1, intLS1 + 1,"", NULL, NULL, NULL, NULL, NULL, NULL, NULL,NULL);
		for (;;)
		{
			intKZ = getchar() - '0';
			if (intKZ == 1 || intKZ == 0)break;
